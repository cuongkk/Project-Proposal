...
bool KhoHang::containsKeyword(const std::string &keyword, const int& option, const SanPham& sp) const 
{
    ...
    switch (option)
    {
        ...
        case 4: //Tìm kiếm sản phẩm theo mức giá nhập vào trở lên
            return (std::stof(sp.get_money().first) >= std::stof(keyword));
        case 5: // Tìm kiếm sản phẩm còn hàng
            return (sp.get_quantity() > 0); 
        case 6: // Tìm kiếm sản phẩm còn hạn sử dụng
            return isExpired(sp.get_expiry_Date()); 
        case 7: // Tìm kiếm sản phẩm hết hạn sử dụng
            return !isExpired(sp.get_expiry_Date());
        default:
